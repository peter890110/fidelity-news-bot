...
FEEDS = {
    ...
    "commercial_times": {
        "name": "工商時報",
        "url": "https://news.google.com/rss/search?q=site:ctee.com.tw+(財經+OR+股市+OR+經濟+OR+金融+OR+半導體+OR+產業+OR+投資)&hl=zh-TW&gl=TW&ceid=TW:zh-Hant"
    },
    "global_market": {
        "name": "國際與美股財經",
...
篩選與分析規則：
1. **內容聚焦（極度重要）**：
   - 【必須且只能】挑選「總體經濟數據與預測、大盤指數與債市走勢、全球央行貨幣政策、重大地緣政治、重量級產業鏈趨勢（如 AI、半導體整體趨勢）」等具備「宏觀（Macro）廣泛影響力」的「純財經、金融或經濟」硬核新聞。
   - 【嚴格排除散戶炒作與農場文】：絕對不准挑選散戶熱衷的無聊炒作、標題殺人法（Clickbait）、網路鄉民熱議、或是缺乏基本面支撐的純題材炒作新聞。
   - 【嚴格排除民生與微觀新聞】：絕對不准挑選民生消費小事（如：中油油價調整）、一般生活時事、政治口水、非金融市場相關的瑣碎新聞。
