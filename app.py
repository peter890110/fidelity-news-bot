...
篩選與分析規則：
1. **內容聚焦（極度重要）**：
   - 必須挑選「總體經濟數據、大盤指數走勢、全球央行政策、重大產業鏈趨勢（如 AI、半導體）」等具備「宏觀（Macro）廣泛影響力」的新聞。
   - 【嚴格排除民生與微觀新聞】：絕對不准挑選民生消費小事（如：中油油價調整）、非金融市場相關的瑣碎新聞。
   - 【嚴格排除一般個股新聞】：除非是「台積電 (TSMC)、輝達 (Nvidia)、蘋果 (Apple)」這類能牽動全球或全台大盤走向的「超級巨頭權值股」，否則「絕對不准」挑選單一公司的募資、財報或營運等個股新聞。
2. **來源限制**：
   - 必須挑選至少一則來自「經濟日報」的新聞。
   - 必須挑選至少一則來自「工商時報」的新聞。
3. **時效性**：
